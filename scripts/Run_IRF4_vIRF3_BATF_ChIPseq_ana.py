...
Para = pu.get_para(configfile="config_IRF4_vIRF3_BATF_ChIPseq.txt")
thread_num = Para[0]
align_idx = Para[1]
out_dir = Para[2]
trimmer_dir = Para[3]
...
fastq_files = pu.get_fastq(configfile="config_IRF4_vIRF3_BATF_ChIPseq.txt")
samples = []
if pair_end == "Yes":
...
## align
for i in range(len(fastq_files)):
    subprocess.run(["./bash/step2_align.sh", thread_num, genome_file_wPath, align_idx, out_dir, bowtie2_dir, alignment_path, pair_end, trimmed_path, fastq_files[i]])
